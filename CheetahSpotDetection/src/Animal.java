 ...
 * counts spots in the image such as images of cheetahs.
 *
...
public class Animal {
  ...
  public static void main(String[] args) {
    ...
    if (mode == 0) {
      ...
      convertToGreyscale(pic, picName, mode);
    } else if (mode == 1) {
      if (args.length != 2) {
        System.err.println("ERROR: invalid number of arguments");
        System.exit(0);
      }
      noiseReduction(pic, picName, mode);
    } else if (mode == 2) {
      if (args.length != 3) {
        System.err.println("ERROR: invalid number of arguments");
        System.exit(0);
      ...
      edgeDetection(pic, picName, epsilon, mode);
    } else if (mode == 3) {
      if (args.length != 5) {
        System.err.println("ERROR: invalid number of arguments");
        System.exit(0);
      ...
      int upperLimit = 0;
      try {
        lowerLimit = Integer.parseInt(args[3]);
        upperLimit = Integer.parseInt(args[4]);
      } catch (Exception e) {
        System.err.println("ERROR: invalid argument type");
        System.exit(0);
      }
   ...
   * @param mode The current mode that the program is running
   */
  public static void convertToGreyscale(Picture pic, String picName, int mode) {
   ...
   * @param mode The current mode that the program is running
   */
  public static void noiseReduction(Picture pic, String picName, int mode) {
    convertToGreyscale(pic, picName, 1);
  ...
  /**
   * This method takes in an image, the image name (without the file extension)
   * and an epsilon value. It then captures the greyscale value of each pixel
   * in the Von Neumann neighborhood. Finally, it determines the difference
   * between each pixel in the Von Neumann neighborhood to the centre pixel
   * and checks whether the difference is less than epsilon and sets the value
   * of the centre pixel accordingly in order to mark the edges within the
   * image. The last double for loop at the end changes the pixels on the far
   * right border and the bottom-most border of the image to black.
  ...
  public static void edgeDetection(Picture pic, String picName,
   double epsilon, int mode) {
    ...
    if (mode == 2) {
      edges.save("../out/" + picName + "_ED.png");
    }
  }

  /**
   * Takes in a radius, width and delta value. Creates a square 2D
   * int array with values corresponding to either a white pixel or
   * black pixel which forms an optimal white spot/circle to compare
   * to image blocks from the edges image in order to help find spots.
   *
   * @param radius Radius
   * @param width Width
   * @param delta Delta
   ...
   * image. The last double for loop at the end changes the pixels on the far
   * right border and the bottom-most border of the image to black.
   * The new image called 'edges' is then returned as output at the end of the
   * method.
   *
   * @param pic The input image
   * @param picName The image name (without the file extension)
   ...
   * and the createSpotMask method to create a mask in order to compare that to
   * an image block of the same size over the edges image. If the image block
   * is similar enough to the mask then it gets added to the spots image and the
   * number of found spots increases.
